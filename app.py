    ...
    if session.get('email'):
        ...
        if request.method=='POST':
            ...
            if count==0:
                ...
                body=f"Rest link  for SPM application : {url_for('reset',data=token(data=email),_external=True)}"
                sendmail(to=email,subject=subject,body=body)
    ...
    try:
        ...
        if request.method=='POST':
            ...
            if npassword==cpassword:
                ...
                mydb.commit()
                cursor.close()
                flash('Newpasword update successfully')
                return redirect(url_for('login'))
            else:
    ...
    if session.get('email'):
        if request.method=='POST':
            ...
            if (pattern.match(name)):
                ...
                cursor.execute('select file_name,file_data from files_data where f_id=%s and added_by=%s',[fid,session.get('email'),name+'%'])
